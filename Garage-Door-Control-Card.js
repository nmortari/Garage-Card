...
class GarageDoorControlCard extends HTMLElement {
  ...
  async command(entityId, action) {
    ...
    if (action !== "stop" && this.config.confirm_actions) {
      ...
      this.render();
      this.shadowRoot.querySelector(".confirm-dialog")?.showModal();
      return;
    }
  ...
  button(entityId, action, label, icon, enabled, emphasis = false) {
    return `<button class="action ${emphasis ? "primary" : ""}" data-entity="${this.escape(entityId)}" data-action="${action}" ${enabled ? "" : "disabled"}>
  ...
  door(item) {
    ...
    const state = String(entity?.state || "unavailable").toLowerCase();
    ...
    return `<section class="door ${info.tone}">
      <div class="door-main">
        <div class="door-icon"><ha-icon icon="${info.icon}"></ha-icon></div>
        <div class="door-copy">
      ...
      <div class="actions">
        ${this.button(item.entity, "open", "Open", "mdi:arrow-up", available && !moving && state !== "open", state === "closed")}
        ${this.button(item.entity, "stop", "Stop", "mdi:stop", available && moving)}
        ${this.button(item.entity, "close", "Close", "mdi:arrow-down", available && !moving && state !== "closed", state === "open")}
      </div>
    </section>`;
  }
  ...
  bindEvents() {
    for (const button of this.shadowRoot.querySelectorAll("button[data-action]")) {
      button.onclick = () => this.command(button.dataset.entity, button.dataset.action);
    }
